<script lang="ts">
	...
	import { themeMode, colors, frameworksTblData, clTblData } from '../../stores';
	...
	import DownloadDocIcon from '$lib/assets/icons/DownloadDocIcon.svelte';
	import TableCallout from './shared/TableCallout/index.svelte';

	import bioImageSm from '$lib/assets/images/dl-bio-photo-128px.jpg';
	import bioImageMd from '$lib/assets/images/dl-bio-photo-256px.jpg';
	import bioImageLg from '$lib/assets/images/dl-bio-photo-384px.jpg';
			...
			<div>
				{#if transitionHeadings}
					<div class="intro-text sm:flex">
						<div in:fly|global="{transitionConfig(25, 350, 0, 10)}" class="bio-photo-container">
						...
						<div class="sm:inline-block ml-0 sm:ml-6">
							<h2
								in:fly|global="{transitionConfig(75, 420, 0, 10)}"
								class="sm:pt-[1.6rem] text-edlDeepBlue dark:text-white"
							>
								About me ⚒️
							</h2>
							<h3 in:fly|global="{transitionConfig(100, 520, 0, 10)}" class="mt-2 text-edlOrange">
								Bridging Functionality & Delight
							</h3>
						</div>
					</div>
				{/if}
				<div class="animate-transition-content"></div>
				<div
					class="mt-8 sm:grid sm:grid-cols-16 sm:gap-6 text-ashenHighContrast-light dark:text-ashenMidContrast-dark"
				>
					<div
						in:fly|global="{transitionConfig(75, 520, 0, 10)}"
					...
					>
						...
						<p>
							I bring prototypes and user interfaces to life by combining my knowledge in javascript
							and web design. Following best practices, I aim to create flexible and reusable
							components that improve maintainability and scale well over time. My background in
							design also helps me identify areas that might cause issues when integrating with dev
							requirements, and create solutions to help avoid those problems.
						</p>
					</div>
					<div
						in:fly|global="{transitionConfig(120, 565, 0, 10)}"
					...
					>
						...
						<p>
							...
							company, and smaller businesses that focus in niche markets. Most of my freelance work
							consists of developing websites, web apps, designing UI, and on occasion providing
							branding services. The UCF Coding Bootcamp has given me a solid foundation in
							front-end web development, and I continue grow my knowledge through workshops, online
							community groups and tech events.
						</p>
					</div>
					<div
						in:fly|global="{transitionConfig(145, 610, 0, 10)}"
					...
					>
						...
						<TableCallout
							...
							tableData="{$frameworksTblData.data}"
							showCols="{false}"
						/>
						<p class="font-niveauGrotesk font-medium text-sm mt-4 dark:text-ashenHighContrast-dark">
							Currently learning:
